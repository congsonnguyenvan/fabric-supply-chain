...
// get history of rubber
func (s *SmartContract) QueryRubberHistory(ctx contractapi.TransactionContextInterface, rubberBatchNumber string) (string, error) {
	rubberCertAsBytes, err := ctx.GetStub().GetHistoryForKey(rubberBatchNumber)

	if err != nil {
		return "", fmt.Errorf("Failed to read from blockchain %s", err.Error())
	}

	defer rubberCertAsBytes.Close()

	var buffer bytes.Buffer
	buffer.WriteString("[")

	bArrayMemberAlreadyWritten := false
	for rubberCertAsBytes.HasNext() {
		response, err := rubberCertAsBytes.Next()
		if err != nil {
...
func (s *SmartContract) QueryRubberBillHistory(ctx contractapi.TransactionContextInterface, billNumber string) (string, error) {
	rubberBillAsBytes, err := ctx.GetStub().GetHistoryForKey(billNumber)

	if err != nil {
		return "", fmt.Errorf("Failed to read from blockchain %s", err.Error())
	}

	defer rubberBillAsBytes.Close()

	var buffer bytes.Buffer
	buffer.WriteString("[")

	bArrayMemberAlreadyWritten := false
	for rubberBillAsBytes.HasNext() {
		response, err := rubberBillAsBytes.Next()
		if err != nil {
			return "", fmt.Errorf(err.Error())
		}
		if bArrayMemberAlreadyWritten == true {
...
func (s *SmartContract) QueryApprovalCertHistory(ctx contractapi.TransactionContextInterface, approvalCertNumber string) (string, error) {
	ApprovalCertAsBytes, err := ctx.GetStub().GetHistoryForKey(approvalCertNumber)

	if err != nil {
		return "", fmt.Errorf("Failed to read from blockchain %s", err.Error())
	}

	defer ApprovalCertAsBytes.Close()

	var buffer bytes.Buffer
	buffer.WriteString("[")

	bArrayMemberAlreadyWritten := false
	for ApprovalCertAsBytes.HasNext() {
		response, err := ApprovalCertAsBytes.Next()
		if err != nil {
			return "", fmt.Errorf(err.Error())
		}
		if bArrayMemberAlreadyWritten == true {
